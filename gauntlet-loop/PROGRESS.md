...
_Generated 8/5/2026, 21:45:32 — refreshes every 30 min while the driver is up._

**Driver:** 🟢 running   ·   **Run:** `2026-08-05_165434`   ·   **HEAD:** `3c2dc89` (80 commits ahead of `origin/main`, unpushed)   ·   **Push guard:** 🔒 armed
...
**Every shard swept.** The loop is now re-sweeping until a pass changes nothing — currently back on `r1c1`.

## Now

```
pass 2  shard r1c1 (162 buildings)
elapsed ██▋·························  9%   3m of ~26m est
```

- phase: **screening (Grok, Cursor Models pool)** — 3m in this phase
- shard started 21:43:01
...
|---|---|---|---|
| **best** | pass-1 tail, then one sweep changes nothing | 3h56m | Thu 01:41 |
| **likely** | pass-1 tail, one fixing sweep, one clean sweep | 7h52m | Thu 05:37 |
| **worst** | pass-1 tail, then all 3 passes, never converges | 11h47m | Thu 09:33 |

One full 9-shard sweep is **3h56m** at the fitted rate.

## What landed

80 commits ahead of `origin/main`, none pushed:

- `3c2dc89` 21:38 — Give unnamed rings the same epoch courtesy as named buildings
- `7c0aa36` 21:30 — Gauntlet r0c2 pass-2: put Scripps-east post-2014 rings in POST_2014_OSM_RINGS.
...
**12 shards judged without Fable** — these carry less judgement than the rest and should be re-audited first:
...
- pass 2 `r0c2` — judged by `cursor-grok-4.5-high` (tier 2, 2/1/1)
...
| ⬜ | Campus footprint measured, not guessed | **99.22% of the footprint inside the campus boundary is measured.** 23 unnamed ring(s) still render at a guess, 5,016 m² — mostly sheds and kiosks the 2014 flight cannot resolve (under canopy, too few returns, or past the survey edge), each refused for a recorded reason. Rings outside the boundary are city, not campus, and are excluded on purpose. Measured 8 min ago. |
| ⬜ | A gauntlet pass finds nothing | pass 2 closed with **14 finding(s)** across 4/9 shards. Two mechanisms guarantee a non-empty pass: the curated epoch name-lists and the unnamed backlog above are both retired one building at a time. |
| 🔸 | Independent panel passes (4 families) | not run |
| ⬜ | Readiness gate passes (handover to Sahir) | blocked on named buildings on a guess (1 of 322); ON-CAMPUS unnamed rings on a guess (23 (+317 off campus)) — measured 8 min ago |
...
|---|---:|---|---|---|---|---|---:|---:|
| `r0c0` | 103 | done | 2 | 2/4/0 | `cursor-grok-4.5-high` | `229a032` | 28m | 24m |
...
| `r1c1` | 162 | **screen** (re-sweep) | 2 | 1/3/0 | `cursor-grok-4.5-high` | `96c1f29` | 23m | 26m |
| `r1c2` | 97 | done | 2 | 0/2/2 | `cursor-grok-4.5-high` | `516760c` | 23m | 24m |
...
Fitted over **45 finished shards** from routed (screen+judge) shards:

```
minutes ≈ 21.1 + 0.033 × buildings
typical miss: ±23 min
```
