...
// and its surveyed boundary read at a glance — and roam free from there. The
// guided walk is one keypress away, and a click on the minimap teleports you
// to that exact spot on the ground below it.
...
export async function boot() {
  ...
  let arcgis, colors, facades, landmarkData, boundary;
  [campus, lidar, arcgis, colors, facades, landmarkData, boundary] = await Promise.all([
  ...
  /* A click on the minimap teleports you to that spot: free roam, same
     ...
     arithmetic (the same transform its own dot is drawn with); this callback
     owns what "being there" means. */
  minimap = createMinimap(document.getElementById("walk-minimap"), {
    colors, lidar, route, boundary,
    onTeleport(x, z) {
      if (state.mode !== "free") setMode("free");
      explore.enterAt(x, z, explore.yaw);
      updateChrome();
    },
  });
  ...
  window.__campusWalk = {
    ...
    /* The same jump a minimap click makes, so a teleport is scriptable end
       to end. */
    teleport(x, z) {
      if (state.mode !== "free") setMode("free");
      explore.enterAt(x, z, explore.yaw);
  ...
  /* SPAWN: hanging SPAWN_ALTITUDE_M over Argo Hall, looking north across the
     ...
     until you press a key — so the first frame is the surveyor's view: the
     campus, its boundary, and the walk about to start below you. */
