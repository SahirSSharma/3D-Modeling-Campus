...
measured may ever read from it.

**The ultra standard (amended 2026-08-17, Sahir's rule).** A treated building is
never partially detailed: every face, band and ground plane it touches ships
complete, or the treatment does not ship. For each face the source ladder is
climbed in order — dated photographs → Street View → drone/video frames →
planning documents and architect drawings → archives — and only when every rung
genuinely fails may the documented pattern of the same building's sourced faces
be extended across the gap, labelled `[estimated]` in `campus-photo-detail.json`.
Estimated detail is the third declared invented tier: it lives inside the photo
class quarantine, records which sourced pattern it extends, and is replaced the
day a real source turns up. Two companions to the rule: nothing may hover or
intersect — every placed thing seats on `surfaceAt` or on the structure that
carries it — and material texture (normal/roughness/albedo-variation maps) is
generated by code, never draped from a photograph or satellite pixel; sourced
colours still feed every material.
